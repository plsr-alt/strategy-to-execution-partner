...
def set_para_text(para, text, size=None, bold=None, color=None):
    p_elem = para._p
...
# 現構成: [0]表紙 [1]はじめに [2]前回VR [3]Win互換 [4]HA [5]FSxW単価
#         [6]費用サマリ [7]利用料概算 [8]今後section [9]確認事項 [10]スケジュール [11]裏表紙
...
for shape in sl.shapes:
    ...
    elif 'コンテンツ' in shape.name or ('プレースホルダー' in shape.name and 'スライド' not in shape.name):
...
# ===== STEP5: 費用サマリ(idx=6) 20TB→50TB(暫定) =====
...
for shape in prs.slides[10].shapes:
    if not shape.has_text_frame: continue
    nm = shape.name
    if 'コンテンツ' in nm or ('プレースホルダー' in nm and 'スライド' not in nm):
        set_run(shape.text_frame, [
...
# ===== STEP8: NWセクションヘッダー を 新規追加(末尾→後でリオーダー) =====
# レイアウト1「中表紙－パターン1」でセクションヘッダー作成
layout_sec = prs.slide_layouts[1]
nw_header_slide = prs.slides.add_slide(layout_sec)
for shape in nw_header_slide.shapes:
    if shape.has_text_frame:
        nm = shape.name
        if 'タイトル' in nm or 'title' in nm.lower():
            tf = shape.text_frame; tf.clear()
            p = tf.paragraphs[0]; run = p.add_run()
            run.text = "NW切替支援について"
            run.font.bold = True

# ===== STEP9: NWコンテンツスライド 新規追加 =====
layout_con = prs.slide_layouts[2]
nw_content_slide = prs.slides.add_slide(layout_con)
for shape in nw_content_slide.shapes:
    ...
    elif 'コンテンツ' in nm or ('プレースホルダー' in nm and 'スライド' not in nm):
        set_run(shape.text_frame, [
...
# ===== STEP10: スライド順序を整える =====
# 現在の順序（0-based）:
#  0=表紙 1=はじめに 2=前回VR 3=Win互換 4=HA 5=FSxW単価
#  6=費用サマリ 7=利用料概算 8=今後section 9=確認事項 10=スケジュール 11=裏表紙
#  12=NW-header(新) 13=NW-content(新)
# 目標順序:
...
# after: NW-header@3, NW-content@13 (shifted)
