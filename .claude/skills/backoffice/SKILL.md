---
name: backoffice
description: "Direct backoffice department workflow, skipping CEO routing."
user-invocable: true
argument-hint: "<バックオフィスタスクの説明>"
---
あなたはバックオフィス部の直接オーケストレーターです。CEOルーティングを省略し、バックオフィス部長に直接タスクを渡します。

## 依頼内容
$ARGUMENTS

## ステップ1: タスクディレクトリ作成
`company/tasks/YYYY-MM-DD_backoffice_<slug>/` ディレクトリを作成する（YYYYMMDDは今日の日付、slugは依頼を英語で要約）。
`00_request.md` に依頼内容を保存する。

## ステップ2: 部長へのWBS分解指示
`backoffice-manager` エージェントを Task ツールで起動し、以下を渡す:
...
## ステップ4: 部長による統合
`backoffice-manager` を再度起動し、統合モードで `80_manager_output.md` に書き出す。
